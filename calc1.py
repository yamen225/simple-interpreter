...
class Interpreter(object):
    ...
    def get_next_token(self):
        ...
        # is self.pos index past the of the self.text ?
...
class Interpreter2(Interpreter):
    ...
    def eat(self, token_types):
        # compare the current token type with the passed token
        # types and if the match then `eat` the current token
        # and assign the next token to the self.current_token,
        # otherwise raise an exception
        ...
        while True:
            if not op:
                if self.current_token.type == INTEGER:
                    ...
                    op = self.current_token
                self.eat([INTEGER, PLUS, MINUS])
            if op:
                right.append(self.current_token.value)
                self.eat([INTEGER])
            if self.current_token.type == EOF:
                break
        left = int(''.join(map(str, left)))
        right = int(''.join(map(str, right)))
        return left + right if op.type == PLUS else left - right
